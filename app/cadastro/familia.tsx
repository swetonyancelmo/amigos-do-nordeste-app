...
import { COMUNIDADES } from '@/dados/comunidades';
...
const OPCOES_COMUNIDADE = [
  ...COMUNIDADES.map(c => ({ valor: c.id, titulo: c.nome })),
...
export default function Familia() {
  ...
  const [gravador] = useState(() =>
    criarGravador<{ id: string; form: Formulario }>(
      async ({ id, form }) => {
        ...
        await salvarRascunho({ id, ...paraBanco(form), pessoas: atual?.pessoas ?? [] }, 'RASCUNHO');
  ...
  useEffect(() => {
    ...
    (idDaRota ? buscar(idDaRota) : buscarRascunhoAberto())
      .then(existente => {
        if (!vivo) return;
        const inicial = existente ? doBanco(existente) : VAZIO;
...
function paraBanco(f: Formulario) {
  const daLista = COMUNIDADES.find(c => c.id === f.comunidade);
  return {
...
function doBanco(p: PreCadastro): Formulario {
  const daLista = COMUNIDADES.find(c => c.id === p.comunidadeId);
  // id que saiu da lista numa atualização do app cai em "Outra", com o nome
